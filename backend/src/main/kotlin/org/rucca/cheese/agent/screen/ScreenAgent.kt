 ...
 *               driver's cheeselet type it into the program's terminal (`say`), and it answers by
 *               running `mat api post-note` back through the tool-door as itself.
 *
 *  Author(s):
...
import org.rucca.cheese.agent.Agent
...
class ScreenAgent(
     ...
     * be heard in a group is `mat api post-note`, and nothing typed into the terminal reaches the
     * group by itself.
     */
    private fun prompt(threadId: IdType, speaker: String, text: String): String =
        "[thread:$threadId] $speaker：$text\n" +
            "（这是群聊消息。请用命令 `mat api post-note 'text: 你的回复'` 把回复发回本群" +
