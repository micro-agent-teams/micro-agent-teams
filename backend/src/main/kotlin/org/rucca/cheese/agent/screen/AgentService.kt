 ...
 *               since nt does not own registration and an agent authenticates by machine+screen
...
package org.rucca.cheese.agent.screen

import at.favre.lib.crypto.bcrypt.BCrypt
import java.time.LocalDateTime
import java.util.UUID
import org.rucca.cheese.agent.AgentRegistry
import org.rucca.cheese.agent.driver.AgentDriver
...
import org.rucca.cheese.team.machine.TeamMachineService
...
class AgentService(
    ...
    @Value("\${application.agent-email-domain:agents.mat.local}")
    ...
    fun openAgent(
        ...
        // The screen's `mat api` authenticates as this machine (CHEESE_TOKEN) against this server
        // (CHEESE_API); paired with the per-screen CHEESE_SCREEN the CLI injects, the tool-door
        // attributes the call to this agent. We inject CHEESE_TOKEN explicitly (the reference
        ...
        val env = buildMap {
            put("CHEESE_API", connectorOrigin)
            machineService.tokenOf(machineId)?.let { put("CHEESE_TOKEN", it) }
        }
        val screen =
            hub.openScreen(
                machineId = machineId,
                ...
                cheeseletSource = driver.cheeseletSource,
                env = env,
            )
