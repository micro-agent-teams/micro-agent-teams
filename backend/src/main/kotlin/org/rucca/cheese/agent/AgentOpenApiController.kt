 ...
 *               CLI's `mat api` (Restish loads it live and turns each operation into
 *               `mat api <operation>`). The backend is OpenAPI-first at BUILD time but does
 ...
 * AgentApi generates from MAT-API.yml: this document is what `mat api` builds its commands from, so
 * a drift here is not a compile error but a live agent whose every reply 404s. AgentLoopTest pins
 * the two together.
 */
