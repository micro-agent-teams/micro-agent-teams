 ...
 *
 *               Authentication is by the machine token in the `X-Cheese-Session` handshake
...
package org.rucca.cheese.machine

import com.fasterxml.jackson.databind.ObjectMapper
import org.rucca.cheese.auth.AuthorizationService
import org.rucca.cheese.machine.enrollment.MachineService
import org.rucca.cheese.machine.link.LinkHandler
import org.rucca.cheese.machine.link.MachineHub
import org.rucca.cheese.machine.screen.ViewerHandler
...
/**
 * Authenticates a machine control-channel handshake from its `X-Cheese-Session` header. The
...
class MachineHandshakeInterceptor(private val machineService: MachineService) :
    ...
    override fun beforeHandshake(
        ...
        val token = request.headers.getFirst("X-Cheese-Session")
        if (token.isNullOrBlank()) {
            logger.warn("machine handshake rejected: missing X-Cheese-Session")
            return false
        }
