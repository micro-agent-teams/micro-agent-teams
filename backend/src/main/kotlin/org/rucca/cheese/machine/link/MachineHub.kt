 ...
 *               server opens *screens* (each a hosted CLI + cheeselet), relays a
 *               screen's raw terminal to browser viewers, mirrors the cheeselet's
 *               variables, answers the cheeselet's calls, and runs one-shot `exec`
...
package org.rucca.cheese.machine.link
...
/** A cheeselet→server function: (hub, screen, args) -> value. Exceptions become rpc errors. */
 ...
 * optionally a cheeselet driving it.
 *
...
class HubScreen(
    ...
    val token: String, // the CHEESE_SCREEN value injected into the screen
    val kind: String,
) {
...
class MachineHub(screenFns: Map<String, ScreenFn> = emptyMap()) {
    ...
    /**
     * Open a screen of [kind] on a machine: mint its per-screen token and ship the optional
     * cheeselet, mirroring web-claude's session.create. The token becomes CHEESE_SCREEN inside the
     * screen; [env] is injected into its process environment.
     *
     * [cheeseletSource] is null for a screen nobody drives -- a plain shell is just a command with
     * a terminal, and `source` is omitted from the frame exactly as the Go CLI's omitempty does.
     */
    fun openScreen(
        machineId: String,
        command: List<String>,
        kind: String,
        cheeseletSource: String? = null,
    ...
    /** Hot-reload the cheeselet into a running screen — no restart, no lost CLI context. */
    ...
    private fun handleScreenCall(machine: HubMachine, screen: HubScreen?, m: LinkMsg) {
        ...
        if (fn == null || screen == null) {
            ...
            } catch (exc: Exception) { // a cheeselet call must never crash the channel
