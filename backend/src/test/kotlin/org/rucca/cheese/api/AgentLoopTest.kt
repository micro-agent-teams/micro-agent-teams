...
import org.rucca.cheese.machine.enrollment.Machine
import org.rucca.cheese.machine.enrollment.MachineRepository
import org.rucca.cheese.machine.link.MachineHub
import org.rucca.cheese.team.machine.TeamMachine
import org.rucca.cheese.team.machine.TeamMachineRepository
...
constructor(
    ...
    private fun connect(collector: Collector): WebSocketSession {
        ...
        headers.add("X-Cheese-Session", machineToken)
        // session.create ships the full cheeselet source (>8 KB), so the client must accept
        ...
        mockMvc
            .perform(
                post("/agent/note")
                    ...
                    .header("X-Cheese-Screen", screenToken)
                    .contentType("application/json")
    ...
    /**
     * The spec `mat api` builds its commands from must advertise the path the server actually
     * serves. Nothing else checks this: the loop test above posts to the tool-door directly, so a
     * stale path here would compile, pass every test, and only surface as a live agent whose every
     * reply 404s — which is exactly what happened when the tool-door moved to /agent/note.
     */
    @Test
