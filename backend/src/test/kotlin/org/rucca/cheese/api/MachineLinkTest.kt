 ...
 *               port: an enrolled machine dials in with its `X-Cheese-Session` token and
...
constructor(
    ...
    private fun connect(token: String, collector: Collector): WebSocketSession {
        ...
        headers.add("X-Cheese-Session", token)
    ...
    fun deviceHandshakeWelcomeScreenAndRpcRoundTrip() {
        ...
        val screen =
            machineHub.openScreen(
                ...
                kind = "test",
                cheeseletSource = "// cheeselet source",
                env = mapOf("CHEESE_API" to "http://127.0.0.1:8080"),
            )
        ...
        assertEquals("// cheeselet source", create["source"].asText())
        assertEquals("bash", create["command"][0].asText())
        assertEquals(120, create["cols"].asInt())
        assertEquals("http://127.0.0.1:8080", create["env"]["CHEESE_API"].asText())
