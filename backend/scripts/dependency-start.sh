...
#               (shared: cheese-auth owns the "public" tables, nt owns "mat") and
...
# nt's own tables live in the "mat" schema; cheese-auth only owns "public".
docker exec postgres psql -U "$DB_USER" -d "$DB_NAME" -c "CREATE SCHEMA IF NOT EXISTS mat;"

echo "== Starting cheese-auth =="
docker rm -f cheese_auth 2>/dev/null || true
...
for _ in $(seq 1 60); do
    status=$(docker inspect -f '{{.State.Health.Status}}' cheese_auth 2>/dev/null || echo starting)
    if [ "$status" = "healthy" ]; then
        echo "cheese-auth is ready"
        exit 0
    fi
    if [ "$(docker inspect -f '{{.State.Status}}' cheese_auth)" = "exited" ]; then
        docker logs cheese_auth
