...
// Both the long-running host (`cheese run`) and the ad-hoc API caller (`cheese
...
// durable credential minted for this device by `cheese auth login`. WS is an
...
// DefaultPath is where the config lives unless overridden (~/.config/cheese/config.json).
func DefaultPath() string {
	base, err := os.UserConfigDir()
	if err != nil {
		base = os.TempDir()
	}
	return filepath.Join(base, "cheese", "config.json")
}
