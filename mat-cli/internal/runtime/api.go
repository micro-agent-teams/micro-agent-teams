...
// installAPI builds the single global the hosted script sees: `cheese`. It is
...
func (r *Runtime) installAPI() error {
	...
	// __log: default logger the runtime uses; scripts get cheese.log.
	...
	_ = cheese.Set("term", term)

	// --- own: script-owned variable (script r/w, server mirrors) ----------
	_ = cheese.Set("own", func(call goja.FunctionCall) goja.Value {
	...
	_ = cheese.Set("watch", func(call goja.FunctionCall) goja.Value {
		name := call.Argument(0).String()
	...
	_ = cheese.Set("expose", func(call goja.FunctionCall) goja.Value {
		name := call.Argument(0).String()
	...
	_ = cheese.Set("call", func(call goja.FunctionCall) goja.Value {
		name := call.Argument(0).String()
	...
	_ = cheese.Set("log", func(msg string) { log.Printf("script: %s", msg) })

	return vm.Set("cheese", cheese)
}
