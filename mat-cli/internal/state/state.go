...
// PID returns the process id of the running host (the `cheese run` service), or 0
// if none is recorded or the recorded process is no longer alive. Used by `cheese
...
// PID it does not treat an unsignalable (EPERM) process as absent — `cheese uninstall`
