...
proxied at `/api`) and **nt** (teams / documents / chat / machines / agents, proxied at `/nt` —
see `vite.config.ts`).
...
`src/api/` is generated from the repo-root **`MAT-API.yml`** — the same single contract the
...
- **Changed the API? Edit `MAT-API.yml` first**, then regenerate. The backend regenerates on its
  ...
  `MAT-API.yml` while `npm run dev` is already running, restart it (Vite will not re-run the hook).
...
Use the generated APIs through the single configured client in `src/lib/ntApi.ts` — it carries the
`/nt` base path, injects the Bearer token, and retries once through the silent-refresh hook on a
401. Do not construct `Configuration` or call `fetch` against nt anywhere else.

```ts
import { chatApi } from "@/lib/ntApi";
...
- `src/lib/api.ts` — the cheese-auth client. cheese-auth is a separate service with its own
  (enveloped `{code,message,data}`) contract and is **not** in `MAT-API.yml`.
- The 现场 viewer's WebSocket (`src/components/TerminalViewer.tsx`) and the STOMP chat socket.
  ...
  backend registers (`/machine/screen/{sid}`, `/nt/ws`). If those move, they move here too.
